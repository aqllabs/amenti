...
class extends Component {
    ...
    {
        $courseProgresses = auth()->user()->completedLessons()->get();
        $this->progressGroupedByCourse = $courseProgresses->groupBy('course_id')
            ->map(function ($group) {
                return $group->count();
            });
    }
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        @foreach ($courses as $course)
            <flux:card :title="$course->title" class="flex flex-col">
                ...
                <progress value="{{($progressGroupedByCourse->get($course->id) ?? 0) / $course->lessons()->count() * 100}}" max="100" class="progress progress-primary w-full mt-2 mb-4"></progress> 
                <div class="mt-auto">
                    <flux:button href="{{route('academy.course.show', $course)}}" variant="primary" class="!bg-primary !text-white hover:!bg-primary/90 w-full">Start Course</flux:button>
                </div>
            </flux:card>
        @endforeach
    </div>
